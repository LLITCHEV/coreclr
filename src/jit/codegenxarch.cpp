...
void CodeGen::genCallInstruction(GenTreePtr node)
{
    ...
    if (varTypeIsStruct(call->gtType))
    {
        ...
        retSize = EA_ATTR(emitTypeSize(compiler->getEightByteType(call->structDesc, 0)));
        retSize1 = EA_ATTR(emitTypeSize(compiler->getEightByteType(call->structDesc, 1)));
    }
    else
    ...
    insCallReturnRegisterTypes callReturnTypes(retSize FEATURE_MULTIREG_RET_ONLY_ARG(retSize1));
    ...
    if (target != nullptr)
    {
        if (target->isContainedIndir())
        {
            if (target->AsIndir()->HasBase() && target->AsIndir()->Base()->isContainedIntOrIImmed())
            {
                ...
                genEmitCall(GenEmitCallInfo(GenCallTypeToEmit::GEN_EMIT_CALL_DIRECT,
                                            emitter::EC_FUNC_TOKEN_INDIR,
                                            methHnd,
                                            INDEBUG_LDISASM_COMMA(sigInfo)
                                            (void*) target->AsIndir()->Base()->AsIntConCommon()->IconValue() // addr
            ...
            else
            {
                ...
                genEmitCall(GenEmitCallInfo(GenCallTypeToEmit::GEN_EMIT_CALL_INDIRECT,
                                            emitter::EC_INDIR_ARD,
                                            methHnd,
                                            INDEBUG_LDISASM_COMMA(sigInfo)
                                            target->AsIndir() // indir
                                            X86_ARG(argSizeForEmitter),
                                            callReturnTypes,
                                            ilOffset));
            }
    ...
    else if (call->gtEntryPoint.addr != nullptr)
    {
        genEmitCall(GenEmitCallInfo(GenCallTypeToEmit::GEN_EMIT_CALL_DIRECT,
                                    ...
                                    methHnd,
                                    INDEBUG_LDISASM_COMMA(sigInfo)
                                    (void*) call->gtEntryPoint.addr // addr
                                    X86_ARG(argSizeForEmitter),
                                    callReturnTypes,
                                    ilOffset));
    }
                                       ...
                                       )
{
    ...
    insCallReturnRegisterTypes callReturnType(retSize);
