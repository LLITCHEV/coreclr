...
void CodeGen::genConsumePutStructArgStk(GenTreePutArgStk* putArgNode, regNumber dstReg, regNumber srcReg, regNumber sizeReg, unsigned baseVarNum)
{
    ...
    if (op1->gtRegNum != dstReg)
    {
        ...
        // Report the type of the dest as ByRef. It is conservative, but it is an internal pointer of the struct, so report it as such.
        getEmitter()->emitIns_R_S(INS_lea, EA_BYREF, dstReg, baseVarNum, putArgNode->getArgOffset());
    }
    ...
    if (op2->gtRegNum != srcReg)
    {
        if (src->OperIsLocalAddr())
        {
            ...
            // Report the type of the src as ByRef. It is conservative, but it is an internal pointer of the struct, so report it as such.
            getEmitter()->emitIns_R_S(INS_lea, EA_BYREF, srcReg, lclNode->gtLclNum, 0);
        }
        else
        {
...
void CodeGen::genCallInstruction(GenTreePtr node)
{
    ...
    emitAttr emitAttrRet1 = EA_PTRSIZE;
    emitAttr emitAttrRet2 = EA_UNKNOWN;
    ...
    if (varTypeIsStruct(call->gtType))
    {
        ...
        emitAttrRet1 = emitTypeSize(compiler->getEightByteType(call->structDesc, 0));
        emitAttrRet2 = emitTypeSize(compiler->getEightByteType(call->structDesc, 1));
    }
    else
#endif // FEATURE_MULTIREG_RET
    {
        ...
        {
            emitAttrRet1 = EA_GCREF;
        }
        else if (call->gtType == TYP_BYREF)
        {
            emitAttrRet1 = EA_BYREF;
        }
    }

    ReturnRegisterTypes callReturnTypes(emitAttrRet1 FEATURE_MULTIREG_RET_ONLY_ARG(emitAttrRet2));
...
#ifdef _TARGET_X86_
    genEmitCallInfo.setArgSize(argSizeForEmitter);
#endif // _TARGET_X86_
    ...
    if (target != nullptr)
    {
        if (target->isContainedIndir())
        {
            if (target->AsIndir()->HasBase() && target->AsIndir()->Base()->isContainedIntOrIImmed())
            {
                ...
                genEmitCallInfo.setGenCallType(GenCallTypeToEmit::GEN_EMIT_CALL_DIRECT);
                genEmitCallInfo.setEmitCallType(emitter::EC_FUNC_TOKEN_INDIR);

                genEmitCallInfo.setAddr((void*)target->AsIndir()->Base()->AsIntConCommon()->IconValue());
            }
            else
            {
                ...
                genEmitCallInfo.setGenCallType(GenCallTypeToEmit::GEN_EMIT_CALL_INDIRECT);
                genEmitCallInfo.setEmitCallType(emitter::EC_INDIR_ARD);
                genEmitCallInfo.setIndir(target->AsIndir());
            }
        }
        else
        {
            ...
            genEmitCallInfo.setGenCallType(GenCallTypeToEmit::GEN_EMIT_CALL_DIRECT);
            genEmitCallInfo.setEmitCallType(emitter::EC_INDIR_R);
    ...
    {
        genEmitCallInfo.setGenCallType(GenCallTypeToEmit::GEN_EMIT_CALL_DIRECT);
        genEmitCallInfo.setEmitCallType((call->gtEntryPoint.accessType == IAT_VALUE) ? emitter::EC_FUNC_TOKEN : emitter::EC_FUNC_TOKEN_INDIR);

        genEmitCallInfo.setAddr(call->gtEntryPoint.addr);
    }
#endif
    else
    {
        ...
        genEmitCallInfo.setGenCallType(GenCallTypeToEmit::GEN_EMIT_CALL_DIRECT);
        genEmitCallInfo.setEmitCallType(emitter::EC_FUNC_TOKEN);
            ...
            case TYPE_GC_BYREF:  // Is an interior pointer - promote it but don't scan it
                {
                    ...
                    // Report the type of the src and dest as ByRef. It is conservative, but it is an internal pointer of the struct, so report it as such.
                    getEmitter()->emitIns_R_I(INS_add, EA_BYREF, REG_RSI, TARGET_POINTER_SIZE);
                    getEmitter()->emitIns_R_I(INS_add, EA_BYREF, REG_RDI, TARGET_POINTER_SIZE);
                    copiedSlots++;
                    gcPtrCount--;
                    i++;
                }
                                       ...
                                       )
{
    ...
    ReturnRegisterTypes callReturnType(retSize);
