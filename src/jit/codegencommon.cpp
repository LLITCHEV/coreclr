...
{
    assert(emitCallInfo.genCallType != GenCallTypeToEmit::GEN_EMIT_CALL_UNKNOWN);

    assert((emitCallInfo.addr == nullptr) || (emitCallInfo.indir == nullptr));

#ifdef _TARGET_XARCH_
    assert(((emitCallInfo.emitCallType != emitter::EC_FUNC_TOKEN) && (emitCallInfo.emitCallType != emitter::EC_FUNC_TOKEN_INDIR)) || 
           (((emitCallInfo.emitCallType == emitter::EC_FUNC_TOKEN) || (emitCallInfo.emitCallType == emitter::EC_FUNC_TOKEN_INDIR)) &&
           (emitCallInfo.addr != nullptr) && (emitCallInfo.indir == nullptr)));

    assert((emitCallInfo.emitCallType != emitter::EC_INDIR_ARD) || 
           ((emitCallInfo.emitCallType == emitter::EC_INDIR_ARD) &&
           (emitCallInfo.addr == nullptr) && (emitCallInfo.indir != nullptr)));
#endif // _TARGET_XARCH_

    assert((emitCallInfo.emitCallType != emitter::EC_INDIR_R) || 
           ((emitCallInfo.emitCallType == emitter::EC_INDIR_R) &&
           (emitCallInfo.addr == nullptr) && (emitCallInfo.indir == nullptr)));
    ...
    if (emitCallInfo.genCallType == GenCallTypeToEmit::GEN_EMIT_CALL_DIRECT)
    {
        assert(emitCallInfo.indir == nullptr);

        getEmitter()->emitIns_Call(emitter::EmitCallType(emitCallInfo.emitCallType),
    ...
    {
        assert(emitCallInfo.genCallType == GenCallTypeToEmit::GEN_EMIT_CALL_INDIRECT);
        assert(emitCallInfo.indir != nullptr);
