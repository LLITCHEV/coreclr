...
protected:
    ...
    struct GenEmitCallInfo
    {
        ...
        GenEmitCallInfo(CORINFO_METHOD_HANDLE       methHndIn,
                        ReturnRegisterTypes         callReturnTypesIn,
                        IL_OFFSETX                  ilOffsetIn) :
                            genCallType(GenCallTypeToEmit::GEN_EMIT_CALL_UNKNOWN),
                            emitCallType(emitter::EC_COUNT),
                            methHnd(methHndIn),
                            INDEBUG_LDISASM_COMMA(sigInfo(nullptr))
                            addr(nullptr),
                            indir(nullptr),
#ifdef _TARGET_X86_
                            argSize(0),
#endif // _TARGET_X86_
                            callReturnTypes(callReturnTypesIn),
                            ilOffset(ilOffsetIn),
                            base(REG_NA),
                            isJump(false),
                            isNoGC(false)
        ...
        // Setters
        void setGenCallType (GenCallTypeToEmit genCallTypeIn)    { genCallType = genCallTypeIn; }
        void setEmitCallType(int               emitCallTypeIn)   { emitCallType = emitCallTypeIn; }
 
#if INDEBUG_LDISASM
        void setSigInfo      (CORINFO_SIG_INFO* sigInfoIn)       { sigInfo = sigInfoIn; }
#endif // INDEBUG_LDISASM

        void setAddr         (void*             addrIn)          { assert(indir == nullptr); addr = addrIn; }
        void setIndir        (GenTreeIndir*     indirIn)         { assert(addr == nullptr); indir = indirIn; }

#ifdef _TARGET_X86_
        void setArgSize      (ssize_t           argSizeIn)       { argSize = argSizeIn; }
#endif // _TARGET_X86_

        void setBase           (regNumber         baseIn)        { base = baseIn; }
        void setIsJump         (bool              isJumpIn)      { isJump = isJumpIn; }
        void setIsNoGC         (bool              isNoGCIn)      { isNoGC = isNoGCIn; }

        GenEmitCallInfo() = delete;
        GenEmitCallInfo(const GenEmitCallInfo& emitCallTypeInfo) = default;

        GenCallTypeToEmit           genCallType;
        int                         emitCallType;
    ...
    void                genEmitCall(const GenEmitCallInfo& emitCallInfo);
